...
public:
  MHARunner(const int numHeads, const int headSize, const int wordSize,
      ...
      : mS(0), mB(0), mOmatSize(0), mNumMats(0), mNumHeads(numHeads),
        mHeadSize(headSize), mWordSize(wordSize), mLdQKV(0), mStrideQKV(0),
        mLdOut(0), mStrideOut(0),
  ...
  virtual void setup_flags(const bool interleaved, const bool ignore_b1opt,
                           const bool force_unroll,
                           const bool use_int8_scale_max, const bool use_tma) {
    return;
  };

  virtual void run(const void *input, const void *mask, void *workspace,
                   void *output, cudaStream_t stream) = 0;
  virtual void run(const void *input, const void *mask, const void *seqlen,
                   void *workspace, void *output, cudaStream_t stream) = 0;
  virtual void run(const void *input, const void *mask,
                   const void *relative_position_bias, const int actual_seqlen,
                   void *workspace, void *output, cudaStream_t stream) = 0;
  virtual void run_causal_masked_fmha(const void *input, const void *cu_seqlens,
                                      void *output, bool causal_mask,
                                      cudaStream_t stream) {
    // unimplemented
    ;
  }

  virtual void setScaleList(const float scaleQkv, const float dqProbs,
                            const float scaleCtx) = 0;
  ...
  virtual int
  getSFromMaxSeqLen(const int max_seq_len,
                    const bool withRelativePositionBias = false) = 0;
...
public:
  ...
  void run(const void *input, const void *mask, void *workspace, void *output,
           cudaStream_t stream);
  void run(const void *input, const void *mask, const void *seqlen,
           void *workspace, void *output, cudaStream_t stream) override;
  void run_causal_masked_fmha(const void *input, const void *cu_seqlens,
                              void *output, bool causal_mask,
                              cudaStream_t stream) override;
  void run(const void *input, const void *mask,
           const void *relative_position_bias, const int actual_seqlen,
           void *workspace, void *output, cudaStream_t stream) override;

  void setScaleList(const float scaleQkv, const float dqProbs,
                    const float scaleCtx) override;
  ...
  int getSFromMaxSeqLen(const int max_seq_len,
                        const bool withRelativePositionBias = false) override;
...
public:
  FusedMHARunnerFP8v2(const int numHeads, const int headSize, const int sm,
                      const float q_scaling);
  ...
  void run(const void *input, const void *mask, const void *seqlen,
           void *workspace, void *output, cudaStream_t stream) override;
  void run(const void *input, const void *mask,
           const void *relatice_position_bias, const int actual_seqlen,
           void *workspace, void *output, cudaStream_t stream) override;
